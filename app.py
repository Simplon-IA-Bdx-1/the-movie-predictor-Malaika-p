...
def insert_movie_query(movie):
    insert_stmt = (
        "INSERT INTO `movies` (`title`, `original_title`, `duration`, `rating`, `release_date`, `synopsis`) " # , `imdb_score`
        "VALUES (%s, %s, %s, %s, %s, %s)" # , %s
    )
    data = (movie.title, movie.original_title, movie.duration, movie.rating, movie.release_date, movie.synopsis) # , movie.imdb_score
...
import_parser.add_argument('--new', help='Importer les dernières sorties depuis une API', required=False)
    ...
    if args.action == "import":
        ...
        if args.api != None:
            ...
            if (movie):
                movie_id = insert_movie(movie)
                if movie.title == None:
                    print(f"'{movie.original_title}' importé depuis TheMovieDB ! Nouvel ID #{movie_id}")
        ...
        if args.new == "new":
            the_movie_db = TheMovieDB()
            movies_list = the_movie_db.get_list_movies()
